...
public class doctorActivity extends javax.swing.JFrame {
    ...
    private void jLabel8MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabel8MouseClicked
        ...
        String sql="select p.count,p.date,p.id,p.name,p.age,p.gender,p.address,"
                + "p.phone,p.status,p.disease,p.room "
                + "from patient p join appointment a "
                + "on(a.dname='" + findName + "' AND p.name=a.pname)";
        ...
        try {
            ...
            //if(rs.next()){
            //String getPname = rs.getString("a.pname");
            //sendName = getPname;
            patientDetails apdetail = new patientDetails(rs);
            apdetail.setVisible(true);
            dispose();
            /**}
            else{
                JOptionPane.showMessageDialog(null, "NO Appointment's ");
            }**/
