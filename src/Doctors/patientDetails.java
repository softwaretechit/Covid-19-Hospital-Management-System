...
{
    JOptionPane.showMessageDialog(null, name+"Your back in ");
}
    public patientDetails(ResultSet name) {
        initComponents();
        ...
        pSTable.setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
        loadData();
    }
    ...
    String selected = model.getValueAt(row, 3).toString();
            ...
            if (row != -1) {
                ...
                try {
                    String sql="select pname from appointment where pname='"+selected+"' ";
                    ps = connection.prepareStatement(sql);
                    rs = ps.executeQuery();
                    if(rs.next()){
                    Appointment appointment=new Appointment(rs.getString("pname"));
                    appointment.setVisible(true);
                   // model.removeRow(row);
                    dispose();
